...
The native input worker repeats the control identity, window geometry/process, pointer, occlusion, foreground, and expiration checks immediately before mouse-down or wheel input. Mouse movement paths, button timing, and typing behavior are unchanged. Requested hover and post-action screenshots are returned inline. `inputExecuted` records that input was sent; it does not establish task success. If post-observation fails, `postObservationError` explains why: inspect the current state before deciding whether to retry.

Raw pointer presses, scrolling, held movement, and drawing timelines require `surface: { observationId, token, elementId }` from a fresh observation of the canvas or viewport. A tightly bounded region observation may omit `elementId`. Absolute points must stay at least eight physical screen pixels inside the surface, avoiding edge resize handles. The server checks the initial visual state and rejects actionable UI controls, changed foreground, occlusion, or expired evidence before native presses. Select toolbar controls separately with prepare/commit; a drawing batch cannot include toolbar clicks. Surface checks do not establish the meaning of arbitrary pixels or guarantee correctness if an application changes during a stroke. Movement without held buttons and keyboard-only timelines do not require a surface.
