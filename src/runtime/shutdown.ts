...
import { terminatePowerShell } from '../windows/powershell';
import { terminateNativeCapture } from '../capture/capture';
...
export const shutdownRuntime = async (state: RuntimeState) => {
  ...
  const workers = await Promise.allSettled([terminateOcr(), terminateOpenCv(), terminatePowerShell(), terminateNativeCapture()]);
